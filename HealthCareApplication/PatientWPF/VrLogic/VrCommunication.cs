...
namespace PatientApp.VrLogic
{
    internal class VrCommunication
    {
        ...
        public static async Task SendAsJson(object payload)
        {
            ...
            var payloadAsBytes = _encoding.GetBytes(JsonConvert.SerializeObject(payload));
        ...
        public static async Task<JObject> ReceiveJsonObject()
        {
            if (!_isConnected)
                throw new CommunicationException(
                    "There is no active communication between the application and the VR server.");
            ...
            var deserializedMessage = JObject.Parse(messageAsString);
